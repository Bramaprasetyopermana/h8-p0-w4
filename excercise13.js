...
// dimana array tersebut berisi objek-objek barang dari toko X tersebut yang berisikan info nama barang, siapa saja yang membeli,
// sisa stock barang dan total pemasukan untuk barang tersebut

//if stock barang < jumlah amount maka batal
...
function countProfit(shoppers) {
  ...
  for (var i = 0; i < listBarang.length; i++) {
    var daftarBarang = {}
    daftarBarang.product = listBarang[i][0]
    daftarBarang.shoppers = []
    daftarBarang.leftOver = listBarang[i][2]
    daftarBarang.totalProfit = 0
    for (var j = 0; j < shoppers.length; j++) {
      if (listBarang[i][0] === shoppers[j].product && listBarang[i][2] >= shoppers[j].amount) {
        daftarBarang.shoppers.push(shoppers[j].name)
        daftarBarang.leftOver -= shoppers[j].amount
        daftarBarang.totalProfit = listBarang[i][1] * (listBarang[i][2] - daftarBarang.leftOver)
      }
    }
    result.push(daftarBarang)
  }
  return result;
}
